...
public class LSVSetReviewToCSVTest {
  ...
  public void canConvertCore19LtdBlueToCSV() throws Exception {
    ...
    assertThat(lines[0]).isEqualTo(
        "Card Name,Colour,Rarity,Rating,Alt Rating,Extra Rating,Raw Ratings,Review");
    assertThat(lines[1]).isEqualTo(
        "Aether Tunnel,U,U,2.5,,,Limited: 2.5,\"This is an neat finisher for aggressive blue decks—it basically guarantees one hit, and if it isn’t answered it clocks the opponent quickly. I’m glad it’s an uncommon, as the joke would get old at common. Not every deck will want this, but those that do will take it as if it were a 3.0 or 3.5—it’s situational but quite powerful.\"");
    assertThat(lines[2]).isEqualTo(
        "Anticipate,U,C,2.0,,,Limited: 2.0,\"Like Opt, this is the kind of card that will often end up on the chopping block because it doesn’t do anything. It’s a good way to spend turn 2 in a deck without low drops, and it smooths out your draws, but you won’t always have space for it. Unlike Opt in Dominaria, there isn’t a heavy spells-matter component, which is why Opt ended up being much stronger in that format. I anticipate playing this most of the time.\"");
    assertThat(lines[37]).isEqualTo(
        "\"Tezzeret, Artifice Master\",U,M,4.5,,,Limited: 4.5,\"Making a Thopter each turn is a nice ability, and Tezzeret starts with enough loyalty that you’ll frequently be able to take the first hit and build up a blocking force. He can even draw extra cards if 1/1 flyers won’t do the trick, and the ultimate is a real thing if they can’t attack his loyalty.\"");
  ...
  public void canConvertCore19LtdGoldToCSV() throws Exception {
    ...
    assertThat(lines[0]).isEqualTo(
        "Card Name,Colour,Rarity,Rating,Alt Rating,Extra Rating,Raw Ratings,Review");
    assertThat(lines[1]).isEqualTo(
        "Aerial Engineer,UW,U,3.0,,,Limited: 3.0,\"Aerial Engineer is a solid body by itself, though not one you’d play without the upside. If you have 2-3 artifacts in your deck, this card is good, and at 4+ it becomes one of your better cards (and I’d adjust the rating up accordingly).\"");
